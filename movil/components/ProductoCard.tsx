...
const ProductoCard = ({
  ...
  return (
    <View className="rounded-lg border border-gray-300 gap-5 mb-5 ">
      <View className="flex-row gap-4 mb-2">
        ...
        <View className="flex-1 min-w-0">
          ...
          <View>
            <Text className="text-yellow-600 text-2xl font-medium">
              $ {precio_venta.toFixed(2)}
            </Text>
            <View className="flex-row px-2 justify-between">
              <Text className="text-md text-white px-2 py-1 rounded-full  bg-black">
                Stock: {stock}
              </Text>
              {marca && (
                <Text className="inline-flex mt-auto border-yellow-200 bg-yellow-50 text-yellow-800 rounded-full px-2.5 items-center text-xs font-medium">
                  {marca}
                </Text>
              )}
            </View>
