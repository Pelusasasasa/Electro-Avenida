...
const ListadoScreen = () => {
  const { productos, startTraerProductosPorTexto, isLoading } =
    useProductoStore();
  ...
  return (
    <View className="mb-5">
      <View className="bg-gradient-to-r bg-yellow-400 from-yellow-400 to-yellow-500 py-3">
        ...
        <View className="mx-2 flex-row gap-2">
          ...
          <Ionicons
            ...
            size={30}
            color={"white"}
            className="bg-black p-2 justify-center rounded-lg"
      ...
      {isLoading ? (
        ...
        <View className="p-4">
          <Text className="text-xl py-2 font-medium">
            Productos Encontrados ({productos?.length})
          </Text>
          <FlatList
            data={productos}
            keyExtractor={(item) => item._id}
            renderItem={({ item }) => <ProductoCard key={item._id} {...item} />}
          />
        </View>
      )}
    </View>
