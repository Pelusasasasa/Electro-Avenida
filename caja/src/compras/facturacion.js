...
async function getFacturas() {
  ...
  const ventas = (
    await axios.get(
      `${URL}ventas/forMonthAndYear/${month}/${year}`,
      configAxios
    )
  ).data;
  return ventas;
...
async function getCompras() {
  ...
  const compras = (
    await axios.get(
      `${URL}dat_comp/forMonthAndYear/${month}/${year}`,
      configAxios
    )
  ).data;
...
async function busqueda() {
  ...
  const facturas = tickets.filter(
    (ticket) => ticket.tipo_comp !== "Nota Credito"
  );
...
async function listarFacturas(facturas) {
  ...
  for (let i = 0; i < facturas.length; i++) {
    ...
    const fechaSig = facturas[i + 1]
      ? new Date(facturas[i + 1].fecha).getDate()
      : -1;
    ...
    if (new Date(factura.fecha).getDate() !== fechaSig) {
      ...
      tdGravado.innerText = redondear(
        totalPrecio - totalIva21 - totalIva105,
        2
      );
...
async function listarCompras(compras) {
  ...
  for (let i = 0; i < compras.length; i++) {
    ...
    tdIva.innerText =
      tipo_comp !== "Nota Credito" ? iva.toFixed(2) : (iva * -1).toFixed(2);
    tdPBrutos.innerText =
...
async function totales() {
  ...
  gravadoT.innerText = redondear(
    parseFloat(gravadoF.innerText) + parseFloat(gravadoN.innerText),
    2
  );
  ...
  total.innerText = redondear(
    parseFloat(gravadoT.innerText) + parseFloat(ivaT.innerText),
    2
  );
  ...
  const auxRetencion =
    parseFloat(document.getElementById("pBru").innerText) +
    parseFloat(document.getElementById("rBru").innerText);
  retencion.innerText = redondear(auxRetencion, 2);

  totalDGR.innerText = redondear(
    parseFloat(dgr.innerText) - parseFloat(retencion.innerText),
    2
  );
  ...
  totalMunicipal.innerText = redondear(
    parseFloat(municipal.innerText) + parseFloat(fdo.innerText),
    2
  );
  ...
  totalIva.innerText = redondear(
    parseFloat(
      ivaT.innerText - parseFloat(document.getElementById("ivaC").innerText)
    ) - parseFloat(document.getElementById("pIva").innerText),
    2
  );
...
async function mostrarAside() {
  ...
  aside.parentNode.style.width === "15px"
    ? (aside.parentNode.style.width = "50%")
    : (aside.parentNode.style.width = "15px");
...
function facturasXLSX(sheetData) {
  ...
  sheetData.push([
    "Total Iva Ventas:",
    redondear(parseFloat(iva21F) + parseFloat(iva105F), 2),
  ]);
