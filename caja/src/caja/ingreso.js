...
select.addEventListener("change", async (e) => {
  const ingresos = (
    await axios.get(
      `${URL}movCajas/forDatesAndIdCuenta/${desde.value}/${hasta.value}/${select.value}`,
      configAxios
    )
  ).data;
  listar(ingresos);
...
const listarRubros = async (cuentasConTipo) => {
  ...
  const ingresos = (
    await axios.get(
      `${URL}movCajas/forDatesAndIdCuenta/${desde.value}/${nextDay}/${select.value}`,
      configAxios
    )
  ).data;
...
const listar = async (lista) => {
  ...
  listaOrdenada.sort((a, b) => {
    //la usamos para ordenar la lsita
