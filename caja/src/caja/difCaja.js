...
async function clickAccionar(e) {
  if (e.target.innerText === "delete") {
    ...
    if (isConfirmed) {
      const mensaje = await axios.delete(
        `${URL}difCaja/id/${seleccionado.id}`,
        configAxios
      );
      if (mensaje) {
    ...
    if (modificar.isConfirmed) {
      const difCaja = (
        await axios.get(`${URL}difCaja/id/${tr.id}`, configAxios)
      ).data;
      ...
      difCaja.diferencia = redondear(
        +difCaja.importe + difCaja.diferencia - aux,
        2
      );
