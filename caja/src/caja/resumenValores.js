...
const confirmarCambios = async (e) => {
  ...
  if (isConfirmed) {
    ...
    ultimos.ceroCincuenta =
      ceroCincuenta.value === "" ? 0 : ceroCincuenta.value;
...
const modificarCambio = () => {
  ...
  cambio.value = redondear(
    cienInput + cincuentaInput + veinteInput + diezInput + monedaInput,
    2
  );
};
...
const ponerValores = (obj) => {
  if (obj) {
    ...
    valesEfectivo.value = redondear(
      parseFloat(chequesEfectivo.value) + parseFloat(totalVales.value),
      2
    );

    diferencia.value = redondear(
      -parseFloat(caja1.value) + parseFloat(valesEfectivo.value),
      2
    );
...
ipcRenderer.on("recibir-informacion", async (e, args) => {
  ...
  hasta = args.hasta;
  const movimientos = (
    await axios.get(`${URL}movCajas/${desde}/${hasta}`, configAxios)
  ).data;
  ...
  valesCobrar.value = (
    await axios.get(`${URL}vales/totalPrice/C`, configAxios)
  ).data.toFixed(2);
  personal.value = (
    await axios.get(`${URL}vales/totalPrice/P`, configAxios)
  ).data.toFixed(2);
  incobrable.value = (
    await axios.get(`${URL}vales/totalPrice/I`, configAxios)
  ).data.toFixed(2);
  facturasCobrar.value = (
    await axios.get(`${URL}vales/totalPrice/F`, configAxios)
  ).data.toFixed(2);
  tarjetasCobrar.value = (
    await axios.get(`${URL}tarjetas/totalPrice`, configAxios)
  ).data.toFixed(2);
  caja1.value = redondear(
    (await axios.get(`${URL}tipoVenta`, configAxios)).data["saldo Inicial"] +
      total,
    2
  );

  totalVales.value = redondear(
    parseFloat(valesCobrar.value) +
      parseFloat(personal.value) +
      parseFloat(incobrable.value) +
      parseFloat(tarjetasCobrar.value) +
      parseFloat(facturasCobrar.value),
    2
  );
