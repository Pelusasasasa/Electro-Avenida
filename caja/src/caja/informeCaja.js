...
window.addEventListener("load", async (e) => {
  ...
  const movimientos = (
    await axios.get(`${URL}movCajas/${desde.value}/${hasta.value}`, configAxios)
  ).data;

  arregloEgresos = movimientos.filter(
    (mov) => mov.pasado && mov.tMov === "E" && mov.pasado === true
  );
  arregloIngresos = movimientos.filter(
    (mov) => mov.pasado && mov.tMov === "I" && mov.pasado === true
  );
...
const listar = async (lista, tbody) => {
  ...
  tbody.appendChild(tr);
  document.querySelector(".saldoAnterior").innerHTML = (
    await axios.get(`${URL}tipoVenta`, configAxios)
  ).data["saldo Inicial"];
  if (tbody.classList.contains("tbodyEgreso")) {
  ...
  document.querySelector(".saldoFinal").innerHTML = redondear(
    totalIngresos -
      totalEgresos +
...
hasta.addEventListener("keypress", async (e) => {
  if (e.key === "Enter") {
    const movimientos = (
      await axios.get(
        `${URL}movCajas/${desde.value}/${hasta.value}`,
        configAxios
      )
    ).data;

    arregloEgresos = movimientos.filter(
      (elem) => elem.pasado && elem.tMov === "E"
    );
    arregloIngresos = movimientos.filter(
      (elem) => elem.pasado & (elem.tMov === "I")
    );
...
document.addEventListener("keyup", (e) => {
  ...
  if (e.keyCode === 17) {
    ...
    const enLista = arregloDeMovimientosAModificar.find(
      (elem) => elem === e.target.parentNode.parentNode.id
    );
    enLista === undefined &&
      arregloDeMovimientosAModificar.push(seleccionado.id);
  }
});
