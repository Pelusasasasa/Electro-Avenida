...
hoy.addEventListener("change", (e) => {
  if (hoy.checked) {
    const tarjetasHoy = tarjetas.filter(
      (tarjeta) => tarjeta.fecha.slice(0, 10) === date.slice(0, 10)
    );
    total = 0;
    listar(tarjetasHoy);
  } else {
...
buscador.addEventListener("keyup", (e) => {
  listar(
    tarjetas.filter((tarjeta) =>
      tarjeta.tarjeta.startsWith(buscador.value.toUpperCase())
    )
  );
});
            ...
            try {
              totalInput.value = redondear(
                parseFloat(totalInput.value) -
                  parseFloat(seleccionado.children[2].innerHTML),
                2
              );
              await axios.delete(
                `${URL}tarjetas/id/${seleccionado.id}`,
                configAxios
              );
