...
const agregarCheque = () => {
  ...
  totalCheque.value = redondear(
    parseFloat(totalCheque.value) + parseFloat(importeCheque.value),
    2
  );
...
const cambiarNumeroComprobantePago = async(lista) => {
  for await (let elem of lista) {
    if (
      elem.children[1].innerText !== "Pago Anticipado" &&
      elem.children[1].innerText !== "Pago A Cuenta"
    ) {
      const comprobante = (
        await axios.get(
          `${URL}ctactePro/numero/${elem.children[0].innerText}`,
          configAxios
        )
      ).data;
...
const descontarSaldoProvedor = async () => {
  ...
  try {
    ...
    sweet.fire({
      title:
        "No se pudo modificar el saldo del provedor, pero si se cargo en la cuenta corriente",
    });
  }
};
...
const ponerEnComprobantePagos = async () => {
  ...
  const tamanio =
    trValores.length > trComprobantes.length
      ? trValores.length
      : trComprobantes.length;
...
const sumarNumeroPago = async () => {
  try {
    await axios.put(
      `${URL}tipoVenta/name/Ultimo Pago`,
      { valor: numeroVenta.value },
      configAxios
    );
  } catch (error) {
    sweet.fire({
      title:
        "No se pudo modifacar el numero de pago, pero si se cargo en la cuenta corriente y se desconto el saldo",
    });
  }
};
...
tbodyCheque.addEventListener("click", (e) => {
  if (e.target.nodeName === "BUTTON") {
    const tr = e.target.parentNode.parentNode;
    totalCheque.value = redondear(
      parseFloat(totalCheque.value) - parseFloat(tr.children[3].innerHTML),
      2
    );
    tbodyCheque.removeChild(tr);
    const index = listaCheques.findIndex(
      (elem) =>
        elem.n_cheque === e.target.parentNode.parentNode.children[0].innerText
    );
...
aceptar.addEventListener("click", async (e) => {
  ...
  if (parseFloat(total.value) === parseFloat(totalCheque.value)) {
    ...
    if (facturas.length !== 0) {
      for await (let fact of facturas) {
        ...
        try {
          ...
          await sweet.fire({
            title:
              "No se pudo pone en cuenta corriente el comprobante de pago en el comprobante  numero " +
              cuenta.nro_comp,
          });
        }
      }
    ...
    for await (let elem of arrayARecorrer) {
      ...
      aux.remitidos = trs2[i]
        ? trs2[i].children[0].innerHTML + " " + trs2[i].children[1].innerHTML
        : "";
...
numero.addEventListener("keypress", async (e) => {
  ...
  if (e.keyCode === 13) {
    facturas = (
      await axios.get(
        `${URL}dat_comp/nro_Comp/${punto + "-" + num}/${codigo.value}`,
        configAxios
      )
    ).data;
    facturas.reverse();
    for await (let { _id, nro_comp, tipo_comp, total: precio } of facturas) {
      if (_id) {
        ...
        total.value =
          tipo_comp === "Nota Credito"
            ? redondear(parseFloat(parseFloat(total.value) - precio), 2)
            : redondear(parseFloat(parseFloat(total.value) + precio), 2);
