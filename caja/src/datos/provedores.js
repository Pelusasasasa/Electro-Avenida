...
nombre.addEventListener("keyup", async (e) => {
  const listaAux = provedores
    ? provedores.filter((provedor) =>
        provedor.provedor.startsWith(nombre.value.toUpperCase())
      )
    : [];
...
codigo.addEventListener("keyup", async (e) => {
  const listaAux = provedores.filter(
    (provedor) => provedor.codigo === codigo.value
  );
  await listar(listaAux);
});
    ...
    await sweet
      ...
      .then(async ({ isConfirmed }) => {
        if (isConfirmed) {
          try {
            await axios.delete(
              `${URL}provedor/codigo/${seleccionado.id}`,
              configAxios
            );
            location.reload();
          } catch (error) {
