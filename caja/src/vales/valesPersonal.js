...
buscador.addEventListener("keyup", (e) => {
  const valeFiltrado = vales.filter((vale) =>
    vale.rsoc.startsWith(e.target.value.toUpperCase())
  );
    ...
    sweet
      ...
      .then(async ({ isConfirmed }) => {
        if (isConfirmed) {
          try {
            await axios.delete(
              `${URL}vales/id/${seleccionado.id}`,
              configAxios
            );
            tbody.removeChild(seleccionado);
            inputTotal.value = redondear(
              parseFloat(inputTotal.value) -
                parseFloat(seleccionado.children[4].innerHTML),
              2
            );
          } catch (error) {
            await sweet.fire({
...
sumar.addEventListener("click", async (e) => {
  ...
  await sweet.fire({
    title: `La suma de ${seleccionado.children[2].innerHTML} es: $${redondear(
      suma,
      2
    )}`,
  });
