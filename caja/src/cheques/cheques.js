  ...
  if (buscador.value !== "") {
    if (porNumero.checked) {
      const chequesFiltrados = cheques.filter((cheque) =>
        cheque.n_cheque.startsWith(buscador.value)
      );
      listarCheques(chequesFiltrados);
    } else if (porRazon.checked) {
      const chequesFiltrados = cheques.filter((cheque) =>
        cheque.ent_por.startsWith(buscador.value.toUpperCase())
      );
      listarCheques(chequesFiltrados);
    } else {
      const chequesFiltrados = cheques.filter((cheque) =>
        cheque.i_cheque?.toString().startsWith(buscador.value)
      );
      listarCheques(chequesFiltrados);
    }
  } else {
    ...
    await sweet
      ...
      .then(async ({ isConfirmed }) => {
        if (isConfirmed) {
          try {
            await axios.delete(
              `${URL}cheques/id/${seleccionado.id}`,
              configAxios
            );
...
const listarCheques = async (cheques) => {
  ...
  for await (let {
    ...
    const fechaCheque = f_cheque
      ? f_cheque.slice(0, 10).split("-", 3)
      : "0000-00-00";
...
function modificarInputs(e, cheque) {
  ...
  console.log(cheque);
  tr.children[0].innerText = cheque.f_recibido
    .split("-", 3)
    .reverse()
    .join("/");
