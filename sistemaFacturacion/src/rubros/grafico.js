...
window.addEventListener("load", async (e) => {
  ...
  const movimientos = (
    await axios.get(
      `${URL}movProductos/${desde.value}/${hasta.value}/${select.value}`,
      configAxios
    )
  ).data;
  const movimientosSinPP = movimientos.filter(
    (movimiento) => movimiento.tipo_pago !== "PP"
  );
...
hasta.addEventListener("keypress", async (e) => {
  if (e.keyCode === 13) {
    ...
    const movimientos = (
      await axios.get(
        `${URL}movProductos/${desde.value}/${nextDay}/${select.value}`
      )
    ).data;
    const movimientosSinPP = movimientos.filter(
      (movimiento) => movimiento.tipo_pago !== "PP"
