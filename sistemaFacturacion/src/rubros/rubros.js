...
const eliminarSubrubro = async (e) => {
  ...
  rubroModificar.subRubros = rubroModificar.subRubros.filter(
    (subRubro) => subRubro !== e.target.parentNode.parentNode.id
  );
  await axios.put(
    `${URL}rubros/${rubroModificar._id}`,
    rubroModificar,
    configAxios
  );
};
...
agregar.addEventListener("click", async (e) => {
  ...
  if (
    !rubroModificado.subRubros.find(
      (elem) => elem === nombreSubRubro.value.toUpperCase().trim()
    )
  ) {
    agregarASubRubro();
    rubroModificado.subRubros.push(nombreSubRubro.value.toUpperCase());
    await axios.put(
      `${URL}rubros/${rubroModificado._id}`,
      rubroModificado,
      configAxios
    );
  }
