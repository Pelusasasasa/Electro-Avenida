    ...
    if (e.keyCode === 17) {
      document.addEventListener(
        ...
        (event) => {
          if (event.keyCode === 67) {
            clipboard.writeText(
              document.querySelector(".subSeleccionado").innerHTML
            );
          }
        },
        { once: true }
...
const buscarPersonaPorCuit = async (valor) => {
  const { nombre, razonSocial, apellido, domicilio } =
    await afip.RegisterScopeThirteen.getTaxpayerDetails(valor);
...
const buscarPersonaPorDNI = async (valor) => {
  ...
  const retorno = {
    ...
    direccion: domicilio ? domicilio[0].direccion : '',
    localidad: domicilio ? domicilio[0].localidad : '',
    provincia: domicilio ? domicilio[0].descripcionProvincia : '',
  };
...
const verificarUsuarios = async () => {
  ...
  await sweet
    ...
    .then(async ({ isConfirmed, value }) => {
      if (isConfirmed) {
        vendedor = (await axios.get(`${URL}usuarios/${value}`, configAxios))
          .data;
      }
    });
  return vendedor;
...
const ponerEnCuentaCorrienteCompensada = async (
  codigo,
  cliente,
  tipo_comp,
  nro_comp,
...
const generarMovimientoCaja = async (
  fecha,
  tMov,
  nro_comp,
  cuenta,
  ...
  (movimiento.codigo = codigo), (movimiento.cliente = cliente);
