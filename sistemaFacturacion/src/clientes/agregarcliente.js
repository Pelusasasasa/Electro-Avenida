...
dnicuit.addEventListener("keypress", (e) => {
  if (e.key === "Enter") {
    if (
      (dnicuit.value.length !== 11 && conIva.value !== "Consumidor Final") ||
      (dnicuit.value.length !== 8 && conIva.value === "Consumidor Final")
    ) {
...
botonEnviar.addEventListener("click", async (e) => {
  ...
  let numero = (
    await axios.get(`${URL}clientes/crearCliente/${inicial}`, configAxios)
  ).data;
  cliente._id = numero;
  try {
