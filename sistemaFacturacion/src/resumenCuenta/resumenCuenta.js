...
buscador.addEventListener('keypress', async (e) => {
  if (e.key === 'Enter') {
    if (buscador.value === '') {
      ...
      cliente = (await axios.get(`${apiUrl}clientes/clienteConSaldo/${buscador.value.toUpperCase()}`, configAxios)).data;
      if (cliente !== '') {
...
const ponerDatos = async (cliente) => {
  ...
  let cuentas = (await axios.get(`${apiUrl}cuentaHisto/cliente/${cliente._id}`)).data;
...
function listarVentas(ventas, situacion, saldoAnterior, saldoAnterior_P) {
  ...
  if (situacion === 'negro') {
    ...
    cliente.cond_iva === 'Inscripto' || cliente.cond_iva === 'Monotributista' ? (aux = 'Factura A') : (aux = 'Factura B');
  }

  let listaAux = ventas;
  if (aux === 'Presupuesto') {
    listaAux = listaAux.filter((e) => {
      return e.tipo_comp === aux || e.tipo_comp === 'Recibos_P' || (e.tipo_comp === 'Descuento: Recibos_P' && e.haber !== 0);
    });
  } else {
    listaAux = listaAux.filter((e) => {
      return e.tipo_comp === aux || e.tipo_comp === 'Recibos' || (e.tipo_comp === 'Descuento: Recibos' && e.haber !== 0) || e.tipo_comp === 'Nota Credito' || e.tipo_comp === 'Ticket Factura';
    });
  }
