...
const retornarClientes = (Clientes, saldo) => {
  const retornar = Clientes.filter(
    (cliente) => cliente[saldo] !== "0" || cliente[saldo] !== "0.00"
  );
  return retornar;
};
...
const listarClientes = async (lista, texto) => {
  ...
  clientes =
    situacion === "blanco"
      ? retornarClientes(clientes, "saldo")
      : retornarClientes(clientes, "saldo_p");
...
tbody.addEventListener("click", (e) => {
  ...
  subSeleccionado && subSeleccionado.classList.remove("subSeleccionado");
  subSeleccionado =
    e.target.nodeName === "TR" ? e.target.children[0] : e.target;
  subSeleccionado.classList.add("subSeleccionado");
});
