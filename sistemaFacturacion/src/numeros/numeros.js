...
async function guardarDatos() {
  ...
  parseFloat(dolarAux) !== parseFloat(dolar.value) &&
    cambiarPrecios(parseFloat(dolar.value));
...
async function cambiarPrecios(dolar) {
  ...
  let productos = (
    await axios.get(
      `${URL}productos/buscarProducto/textoVacio/dolar`,
      configAxios
    )
  ).data;
  productos.sort((a, b) => {
  ...
  const a = productos.filter(
    (producto) => producto.costodolar !== 0 && producto.costodolar
  );
  ...
  for await (let producto of a) {
    ...
    const costoMasIva = parseFloat(
      redondear(
        parseFloat(producto.impuestos) + parseFloat(producto.costodolar),
        2
      )
    );
    const costoTotal = parseFloat(redondear(dolar * costoMasIva, 2));
    producto.precio_venta = Math.round(
      costoTotal + (parseFloat(producto.utilidad) * costoTotal) / 100
    ).toFixed(2);
