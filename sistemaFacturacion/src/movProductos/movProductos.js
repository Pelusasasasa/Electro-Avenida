...
cantidad.addEventListener("blur", (e) => {
  ...
  if (operacion === "Compra" || operacion === "Suma") {
    nuevoStock.value = (
      parseFloat(cantidad.value) + parseFloat(stock.value)
    ).toFixed(2);
  } else {
    nuevoStock.value = (
      parseFloat(stock.value) - parseFloat(cantidad.value)
    ).toFixed(2);
  }
});
...
aceptar.addEventListener("click", async (e) => {
  ...
  operacion === "Resta"
    ? (movProducto.egreso = cantidad.value)
    : (movProducto.ingreso = cantidad.value);
  ...
  let producto = (
    await axios.get(`${URL}productos/${movProducto.codProd}`, configAxios)
  ).data;
  producto.stock = movProducto.stock;
  producto.vendedor = vendedor;
  producto.maquina = movProducto.maquina;
  await axios.put(
    `${URL}productos/${movProducto.codProd}`,
    producto,
    configAxios
  );
