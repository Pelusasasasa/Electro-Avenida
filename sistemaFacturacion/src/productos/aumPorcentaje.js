...
modificar.addEventListener('click',async e=>{
    ...
    await productos.forEach(async producto=>{
        if (producto.costodolar === 0) {
            ...
            producto.costodolar = (parseFloat(producto.costodolar) + parseFloat(producto.costodolar)*porcentaje/100).toFixed(2);
            producto.impuestos = parseFloat(((producto.iva === "N") ? (parseFloat(producto.costodolar) * 26 / 100) : (parseFloat(producto.costodolar) * 15 / 100)).toFixed(2));
            const costoIva = Math.round((parseFloat(producto.costodolar) + parseFloat(producto.impuestos)) * dolar);
