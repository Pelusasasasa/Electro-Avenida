...
select.addEventListener("change", async (e) => {
  const rubro = (await axios.get(`${URL}rubros/${e.target.value}`, configAxios))
    .data;
...
costoDolares.addEventListener("blur", (e) => {
  costoDolares.value !== "" &&
    (costoT = resultado(parseFloat(costoDolares.value), valorTasaIva, dolar));
});
...
costoTotal.addEventListener("focus", (e) => {
  ...
  if (costoPesos.value === "" || parseFloat(costoPesos.value) === 0) {
    costoTotal.value = (
      (parseFloat(costoDolares.value) + parseFloat(ivaImp.value)) *
      dolar
    ).toFixed(2);
  } else {
...
precioVenta.addEventListener("focus", (e) => {
  precioVenta.value = Math.round(
    precioV + parseFloat(costoTotal.value)
  ).toFixed(2);
});
  ...
  if (descripcion.value === "") {
    ...
    if (imagen.files[0]) {
      ...
      const streamEscritura = fs.createWriteStream(
        "\\\\SERVIDOR//imagenes//" + producto._id + ".jpg"
      );
