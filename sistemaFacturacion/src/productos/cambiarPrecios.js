...
window.addEventListener("load", async (e) => {
  ...
  const provedores = (
    await axios.get(`${URL}productos/provedores`, configAxios)
  ).data;
...
select.addEventListener("change", async (e) => {
  if (select.value === "SAN JUSTO") {
    await sweet.fire({
      title:
        "La columna de codigo de fabrica tiene que llamarse CODIGO y la de costo Precio en el EXCEL y el libro se tiene que llamar Lista",
      returnFocus: false,
    });
  }
  if (select.value === "CANDIL") {
    await sweet.fire({
      title:
        "La hoja se debe llamar Lista, y la columca con el costo se tiene que llamar Precio (las tira ed el precio esta por 5 metros en el excel)",
      returnFocus: false,
    });
  } else if (select.value === "BREMEN") {
    await sweet.fire({
      title:
        "La columna de codigo de fabrica tiene que llamarse Código y la de costo PRECIO en el EXCEL Y tambien pasar la fila de idenficaciones como el primer renglon",
      returnFocus: false,
    });
    ...
    await sweet.fire({
      title:
        "Borrar la prime fila dejando como primera la fila de Codigo y Precio(precio final en pesos) y tambien poner el nombre de la hoja como Lista",
      returnFocus: false,
    });
  }
  dolar.focus();
...
archivo.addEventListener("change", (e) => {
  ...
  fileReader.onload = async function (e) {
    ...
    if (select.options[select.selectedIndex].classList.contains("provedor")) {
      productos = (
        await axios.get(
          `${URL}productos/provedores/${select.value}`,
          configAxios
        )
      ).data;
    } else {
      productos = (
        await axios.get(`${URL}productos/marcas/${select.value}`, configAxios)
      ).data;
    }
  ...
  for await (let elem of productos) {
    ...
    if (producto) {
      if (elem.costodolar !== 0) {
        ...
        elem.impuestos = parseFloat(
          redondear((elem.costodolar * tasaIva) / 100, 2)
        );
        const costoIva =
          (elem.costodolar + elem.impuestos) * parseFloat(dolar.value);
        const utilidad = (costoIva * parseFloat(elem.utilidad)) / 100;
        elem.precio_venta = parseFloat(
          Math.round(costoIva + utilidad).toFixed(2)
        );
      } else {
    ...
    if (producto) {
      ...
      if (elem.costodolar !== 0) {
        ...
        const costoIva = parseFloat(
          redondear(elem.costodolar + elem.impuestos) * parseFloat(dolar.value),
          2
        );
        const utilidad = (costoIva * parseFloat(elem.utilidad)) / 100;
        elem.precio_venta = parseFloat(
...
async function cambiarPrecioHaedo(datos, productos) {
  for await (let elem of productos) {
    const producto = datos.find(
      (dato) => dato.Artículo.slice(2) == elem.cod_fabrica
    );
    ...
    if (producto) {
      if (elem.costodolar !== 0) {
        ...
        const utilidad = parseFloat(
          redondear((costoIva * elem.utilidad) / 100, 2)
        );
        ...
        const utilidad = parseFloat(
          ((costoIva * elem.utilidad) / 100).toFixed(2)
        );
        elem.precio_venta = Math.round(costoIva + utilidad);
      }
    }
...
async function cambiarPrecioGomez(datos, productos) {
  for await (let elem of productos) {
    ...
    if (producto) {
      if (elem.costodolar !== 0) {
        elem.costodolar = parseFloat(
          (producto.Precio / parseFloat(dolar.value)).toFixed(2)
        );
        elem.impuestos = parseFloat(
          redondear((elem.costodolar * tasaIva) / 100, 2)
        );
...
async function cambiarPrecioLanus(datos, productos) {
  for await (let elem of productos) {
    const tasaIva = elem.iva === "R" ? 15 : 26;
    const producto = datos.find(
      (dato) => dato.CODIGO?.trim() == elem.cod_fabrica.trim()
    );
