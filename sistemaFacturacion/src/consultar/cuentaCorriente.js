...
const listarLista = (lista, situacion, tipo) => {
  ...
  listaGlobal = lista.filter((e) => {
    if (aux === 'Presupuesto') {
      ...
      return e.tipo_comp === aux || e.tipo_comp === 'Recibos' || e.tipo_comp === 'Descuento: Recibos' || e.tipo_comp === 'Nota Credito' || e.tipo_comp === 'Ticket Factura';
    }
  });
...
actualizar.addEventListener('click', async (e) => {
  if (seleccionado && !seleccionado.classList.contains('detalle')) {
    ...
    arregloRestante = arregloRestante.filter((e) => {
      return e.tipo_comp === 'Presupuesto' || e.tipo_comp === 'Recibos_P' || e.tipo_comp === 'Descuento: Recibos_P';
    });
