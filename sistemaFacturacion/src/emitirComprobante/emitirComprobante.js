...
precioAgregar.addEventListener("keypress", (e) => {
  if (e.key === "Enter" && codigo.value !== "888-888") {
    const product = {
      ...
      precio_venta:
        precioAgregar.children[0].value !== ""
          ? parseFloat(precioAgregar.children[0].value)
          : 0,
      _id: codigo.value,
      marca: "",
      unidad: "",
    ...
    if (e.target.value !== "") {
      if (codigo.value === "999-999") {
        ...
        precio.addEventListener("keypress", (e) =>
          ponerPrecioFinanciacionTarjeta(e, descripcion, precio)
        );
              ...
              if (isConfirmed && value !== "" && value !== ".") {
                if (
                  ...
                  if (
                    ...
                    await sweet.fire({
                      title:
                        "La cantidad de este producto no puede ser en decimal",
                    });
                  } else {
                    producto.stock < 0 &&
                      (await sweet.fire({
...
ipcRenderer.on("mando-el-producto", async (e, args) => {
  ...
  if (id === "777-058") {
    ...
    porcentaje.children[0].addEventListener("keypress", (e) =>
      ponerFinanciacionBancoEntreRios(e, descripcion, porcentaje)
    );
  } else {
  ...
  if (e.key === "Enter") {
    ...
    producto.objeto.precio_venta =
      cambioPrecio.children[0].value !== ""
        ? parseFloat(cambioPrecio.children[0].value)
        : producto.objeto.precio_venta;
    producto.objeto.precioOferta =
      cambioPrecio.children[0].value !== ""
        ...
        : producto.objeto.precioOferta;
    producto.cantidad =
      nuevaCantidad.value !== "" ? nuevaCantidad.value : producto.cantidad;
...
function verQueVentaEs(tipo, cod_comp) {
  if (tipo === "Presupuesto") {
    ...
    tipo === "Factura A" ||
    (tipo === "Ticket Factura" && cod_comp === 1)
  ) {
    return "Ultima Factura A";
  } else if (
    tipo === "Factura B" ||
    (tipo === "Ticket Factura" && cod_comp === 6)
  ) {
...
async function traerUltimoNroComprobante(
  tipoCom,
  codigoComprobante,
  tipo_pago
) {
  ...
  if (
    tipoCom === "Ticket Factura" ||
    tipoCom === "Factura A" ||
    tipoCom === "Factura B"
  ) {
    const numeroFactura = verQueVentaEs(tipoCom, codigoComprobante);
    let tipoVenta = (await axios.get(`${URL}tipoVenta`, configAxios)).data[
      numeroFactura
    ];
    return tipoVenta;
  } else if ((tipoCom === "Presupuesto") & (tipo_pago === "CD")) {
    const numeroFactura = verQueVentaEs("Contado");
    const tipoVenta = (await axios.get(`${URL}tipoVenta`, configAxios)).data[
      numeroFactura
    ];
    return tipoVenta;
...
async function sacarStock(cantidad, objeto) {
  let producto = (await axios.get(`${URL}productos/${objeto._id}`, configAxios))
    .data;
...
async function sumarSaldoAlClienteEnNegro(precio, codigo, valorizado, venta) {
  ...
  let cliente = (await axios.get(`${URL}clientes/id/${codigo}`, configAxios))
    .data;
...
presupuesto.addEventListener("click", async (e) => {
  ...
  if (codigoC.value === "") {
    ...
    (await verProductoConCero(listaProductos)) &&
    !document.getElementById("presupuesto").checked
  ) {
  ...
  try {
    ...
    venta.nro_comp = await traerUltimoNroComprobante(
      tipoVenta,
      venta.cod_comp,
      venta.tipo_pago
    ...
    await actualizarNumeroComprobante(
      venta.nro_comp,
      venta.tipo_pago,
      venta.cod_comp
    );
    ...
    if (venta.tipo_pago === "CC") {
      await sumarSaldoAlClienteEnNegro(
        venta.precioFinal,
        venta.cliente,
        valorizado.checked,
        venta.nro_comp
    ...
    for (let producto of venta.productos) {
      if (venta.tipo_pago !== "PP" && !facturarPrestamo) {
        producto.objeto._id !== "999-999" &&
          (await sacarStock(producto.cantidad, producto.objeto));
      }
      await movimientoProducto(
        producto.cantidad,
        producto.objeto,
        venta.cliente,
        venta.nombreCliente,
...
prestamo.addEventListener("click", async (e) => {
  ...
  let numeros = (await axios.get(`${URL}tipoVenta`, configAxios)).data;
  let ultimoNumeroPrestamo =
    parseInt(numeros["Ultimo Prestamo"].split("-", 2)[1]) + 1;
  prestamo.nro_comp =
    "0007-" + ultimoNumeroPrestamo.toString().padStart(8, "0");
...
ticketFactura.addEventListener("click", async (e) => {
  ...
  if (parseFloat(total.value) < 0) {
    ...
    dnicuit.value.length === 11 &&
    conIva.value === "Consumidor Final"
  ) {
    await sweet.fire({ title: "No se puede Consumidor Final con Cuit" });
  } else if (
    dnicuit.value.length === 8 &&
    conIva.value !== "Consumidor Final"
  ) {
    ...
    if (venta.tipo_pago === "Ninguno") {
      ...
      if (
        ...
        try {
          ...
          const [iva21, iva105, gravado21, gravado105, cant_iva] =
            gravadoMasIva(venta.productos);
          ...
          venta.tipo_pago === "CD" &&
            generarMovimientoCaja(
              ...
              "I",
              venta.nro_comp,
              venta.cod_comp === 1 ? "Factura A" : "Factura B",
              venta.cod_comp === 1 ? "FA" : "FB",
              venta.precioFinal,
              venta.nombreCliente,
              venta.cliente,
              venta.nombreCliente,
          ...
          if (borraNegro) {
            ...
            const movimientosViejos = (
              await axios.get(
                `${URL}movProductos/${ventaAnterior.nro_comp}/Presupuesto`,
                configAxios
              )
            ).data;
            ...
            await descontarSaldo(
              ventaAnterior.cliente,
              ventaAnterior.precioFinal,
              ventaAnterior.nro_comp,
              venta.nro_comp
            );
            await borrarCuentaHistorica(
              ventaAnterior.nro_comp,
              ventaAnterior.cliente,
              ventaAnterior.tipo_comp
            );
            ...
            for await (let numero of listaNumeros) {
              ...
              const movimientosViejos = (
                await axios.get(
                  `${URL}movProductos/${numero}/Presupuesto`,
                  configAxios
                )
              ).data;
              for await (let mov of movimientosViejos) {
...
const borrarUnProductoDeLaLista = async (productoSeleccionado) => {
  if (productoSeleccionado) {
    producto = listaProductos.find(
      (e) => e.objeto.identificadorTabla === productoSeleccionado.id
    );
    ...
    const descuentoProducto = parseFloat(
      (aDescontar * parseFloat(descuento.value / 100).toFixed(2)).toFixed(2)
    );

    total.value = (
      parseFloat(total.value) -
      (aDescontar - descuentoProducto)
    ).toFixed(2);

    Preciofinal =
      Preciofinal -
      parseFloat(productoSeleccionado.children[5].innerHTML).toFixed(2);
    for await (let e of listaProductos) {
      if (productoSeleccionado.id === e.objeto.identificadorTabla) {
        ...
        totalPrecioProductos -= e.objeto.oferta
          ? e.objeto.precioOferta * e.cantidad
          : e.objeto.precio_venta * e.cantidad;
      }
    }
...
ipcRenderer.once("venta", async (e, args) => {
  ...
  ventaAnterior = (await axios.get(`${URL}presupuesto/${numero}`, configAxios))
    .data;
  let cliente = (
    await axios.get(`${URL}clientes/id/${ventaAnterior.cliente}`, configAxios)
  ).data;
...
ipcRenderer.on("informacion", async (e, args) => {
  ...
  for await (let numero of numeros) {
    ventas.push(
      (await axios.get(`${URL}presupuesto/${numero}`, configAxios)).data
    );
  }
...
const descontarSaldo = async (codigo, precio, numero = "", venta = "") => {
  const cliente = (await axios.get(`${URL}clientes/id/${codigo}`, configAxios))
    .data;
...
const borrarCuentaHistorica = async (numero, cliente, tipoComp) => {
  const eliminada = (
    await axios.delete(`${URL}cuentaHisto/id/${numero}`, configAxios)
  ).data;
  const importeEliminado = eliminada.debe;
  const historicas = (
    await axios.get(`${URL}cuentaHisto/cliente/${cliente}`, configAxios)
  ).data;
  let cuentaHistoricas = historicas.filter(
    (historica) => historica.tipo_comp === tipoComp
  );
  cuentaHistoricas = cuentaHistoricas.filter(
    (historica) => historica.fecha > eliminada.fecha
  );
  cuentaHistoricas.forEach(async (cuenta) => {
    cuenta.saldo = cuenta.saldo - importeEliminado;
    await axios.put(
      `${URL}cuentaHisto/id/${cuenta.nro_comp}`,
      cuenta,
      configAxios
    );
  });
...
async function traerMovimientosPrestamo(arreglo) {
  ...
  for (let elem of arreglo) {
    const movimientos = (
      await axios.get(`${URL}movProductos/${elem}/Prestamo`, configAxios)
    ).data;
...
async function traerProductosPrestamo(arreglo) {
  ...
  for await (let { codProd } of arreglo) {
    const producto = (
      await axios.get(`${URL}productos/${codProd}`, configAxios)
    ).data;
  ...
  if (e.key === "Enter") {
    ...
    const product = {
      ...
      precio_venta: parseFloat(
        precio.children[0].value === "" ? 0 : precio.children[0].value
      ),
      _id: codigo.value,
    };
