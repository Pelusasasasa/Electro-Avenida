...
async function mostrarDetalleProducto(e) {
  ...
  /*Traer movimientos*/ const movimientos = (
    await axios.get(
      `${URL}movProductos/${seleccionado.id}/Prestamo`,
      configAxios
    )
  ).data;
...
async function facturarPrestamos() {
  ...
  location.href = `../emitirComprobante/emitirComprobante.html?facturarPrestamo=${true}&arregloPrestamo=${JSON.stringify(
    arrayAFacturar
  )}&observaciones=${seleccionado.children[4].innerText}`;
...
async function imprimirPrestamo() {
  const venta = (
    await axios.get(`${URL}prestamos/forNumber/${seleccionado.id}`, configAxios)
  ).data;
  const cliente = (
  ...
  const movimientos = (
    await axios.get(
      `${URL}movProductos/${venta.nro_comp}/${venta.tipo_comp}`,
      configAxios
    )
  ).data;
...
async function cambiarObservacion() {
  ...
  if (isConfirmed) {
    ...
    await axios.put(
      `${URL}prestamos/forNumber/${prestamo.nro_comp}`,
      prestamo,
      configAxios
    );
