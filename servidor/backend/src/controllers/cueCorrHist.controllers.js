...
cuentaHistoricaCTRL.cargarHistorica = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `No se pudo cargar la cuenta historica de ${req.body.codigo} a la fecha ${req.body.fecha} con el numero ${req.body.nro_comps}`
    );
    console.error(error);
  }
  console.log(
    `Cuenta Historica Nº ${req.body.nro_comp} Del cliente ${
      req.body.cliente
    } cargado por el vendedor ${req.body.vendedor} de la maquina ${
      req.body.maquina
    } con la hora y fecha ${new Date().toLocaleString()}`
  );
...
cuentaHistoricaCTRL.modificarHistorica = async (req, res) => {
  ...
  console.log(
    `Historica ${req.body.nro_comp} del cliente ${req.body.cliente} Modificada`
  );
  res.send(
    `Historica ${req.body.nro_comp} del cliente ${req.body.cliente} Modificada`
  );
...
cuentaHistoricaCTRL.modificarHistoricaPorNumero = async (req, res) => {
  ...
  console.log(
    `Cuenta Historica de ${req.body.cliente} con numero ${req.body.nro_comp} Modificado`
  );
  res.send(
    `Cuenta Historica de ${req.body.cliente} con numero ${req.body.nro_comp} Modificado`
  );
...
cuentaHistoricaCTRL.modificarHistoricaPorId = async (req, res) => {
  const { id } = req.params;
  await CuentaHisto.updateOne({ _id: id }, req.body);
